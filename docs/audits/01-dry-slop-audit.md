...
4. **Branding split: QuotaGuard vs MeterBar vs shipshit** — the public site is `meterbar.dev`, while migration-sensitive internal values remain `dev.shipshit.MeterBar`, `dev.shipshit.MeterBar.Widget`, `group.dev.shipshit.meterbar`, and the `dev.shipshit.meterbar` keychain service. Legacy "Quota Guard" headers remain in [CLAUDE.md](CLAUDE.md)/[generate-social-preview.ts](scripts/generate-social-preview.ts)/[APIAccessTest.swift](scripts/APIAccessTest.swift)/[test-api-access.sh](scripts/test-api-access.sh), along with the `app.meterbar` log fallback. (Bundle, app-group, and keychain IDs require an explicit migration rather than a search-and-replace.)
