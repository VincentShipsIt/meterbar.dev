...
4. **Branding split: QuotaGuard vs MeterBar** — current identity values are `dev.meterbar.app`, `dev.meterbar.app.widget`, `group.dev.meterbar.app`, and `dev.meterbar.app` keychain service, but legacy names remain in `quotaguard-scripts` ([scripts/package.json](scripts/package.json)), "Quota Guard" headers in [CLAUDE.md](CLAUDE.md)/[generate-social-preview.ts](scripts/generate-social-preview.ts)/[APIAccessTest.swift](scripts/APIAccessTest.swift)/[test-api-access.sh](scripts/test-api-access.sh), and `app.meterbar` log fallback.
