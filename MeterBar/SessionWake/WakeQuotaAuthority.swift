...
/// Default provider. It prefers the authenticated `/api/oauth/usage` endpoint —
/// the same source the usage card adopted in PR #175 — because `claude /usage`
/// no longer renders in a headless (non-TTY) spawn and its parse fails. Every
/// account may take that path: the credential is resolved per profile, so a
/// scoped account reads its own token rather than the unscoped identity's. It
/// falls back to the `claude /usage` CLI for a missing/expired token or an OAuth
/// opt-out. The OAuth path is side-effect-free: unlike the usage card's
/// `ClaudeCodeLocalService.fetchUsageMetrics`, it never mutates `@Published` UI
/// state, so background wake polls stay decoupled from the UI.
nonisolated struct LiveWakeQuotaProvider: WakeQuotaProviding {
    /// Fetches usage via OAuth without UI side effects for the given account;
    /// `nil` ⇒ no usable token, so fall back to the CLI. A throw ⇒ a token was
    /// in hand but the request failed — surface it (fail closed) rather than
    /// retry the headless-broken CLI.
    private let oauthMetrics: @Sendable (ClaudeCodeAccount) async throws -> UsageMetrics?
    ...
    init(
        oauthMetrics: @escaping @Sendable (ClaudeCodeAccount) async throws -> UsageMetrics? = { account in
            try await ClaudeCodeLocalService.oauthMetricsWithoutSideEffects(account: account)
        },
    ...
    func fetchMetrics(account: ClaudeCodeAccount) async throws -> UsageMetrics {
        if oauthEnabled() {
            ...
            if let metrics = try await oauthMetrics(account) {
                return metrics
            }
        }
