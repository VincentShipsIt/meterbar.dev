...
// split). Pure move: these primitives are shared by every settings tab (and by
// EmptyStateCard), so they live alongside the other view components rather than
// buried in one 2k-line file. Types that previously had to be `private` in
// SettingsView.swift are now `internal` so the per-tab views can reuse them.
...
enum SettingsInputSurfaceShape {
