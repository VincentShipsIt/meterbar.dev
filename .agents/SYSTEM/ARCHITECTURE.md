...
| Grok | `.grok` | Tracked by default (explicit opt-out only). Runs the official Grok Build CLI's ACP stdio agent with its cached login and maps `_x.ai/billing` onto the shared session/weekly windows plus `ExtraUsageStatus`; MeterBar checks login-file presence but never reads token contents |
...
- **ProviderVisibilityStore / DockVisibilityStore / ClaudeCodeAccountStore / CodexAccountStore / GrokAccountStore** — UserDefaults-backed preference stores. Account stores mirror non-secret profile configuration into the app-group refresh files so the bundled CLI uses the same enabled profiles and ordering.
...
`UsageMetrics.modelLimitLabel` is an additive optional cache field. Older payloads without it decode to `nil`; newer readers use the neutral `Model` fallback rather than mislabeling an unknown/Fable window as Sonnet.
