...
| Codex CLI | `.codexCli` | `$CODEX_HOME/auth.json` (default `~/.codex/auth.json`) → `https://chatgpt.com/backend-api/wham/usage`. Exhausted accounts can spend a banked reset credit after explicit confirmation. `additional_rate_limits` carries separately metered pools: the `base_model_inference` reserve that serves requests once the plan window is spent (verified 2026-09-09, `normal_model_slug: gpt-5.6-luna` → **Luna Reserve**, surfaced only while `rate_limit_upsell.banner_type == luna_reserve` or `limit_reached`), and the Codex Spark windows, which are **not** mapped — two windows share one `limit_name` and need a cadence-composed title (issue #578). |
