...
**Status:** Implementation complete; PR CI and exact-head review pending
...
- The first exact-head Opus review rejected the OAuth mapper because it only decoded `seven_day_sonnet`; the repair adds `seven_day_fable`, preserves the matched label, and adds OAuth contract coverage. A fresh verdict is required on the repair head.
...
- [ ] Commit and publish the ready-for-review PR closing #252.
- [ ] Require green CI and an exact-head Opus PASS before merge.
