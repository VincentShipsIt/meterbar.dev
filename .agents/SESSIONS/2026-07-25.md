...
## Session 16: C1d — split `CostTracker.swift` into scan collaborators

**Status:** Authored in parallel with the audit wave (19:04) but landed last, so
it carries the wave's next ordinal. Master merged into the branch to satisfy
`strict: true`; PR CI + exact-head verification run at that merge head.

Last item of audit finding C1 (four files over 950 lines, one PR each).
`CostTracker.swift` was 1,018 lines: an `ObservableObject` that also owned every
Claude and Codex transcript parser, the pricing formula, the aggregation, and the
summary assembly. Roughly a thousand lines of money math reachable only through a
full filesystem scan, with **zero direct tests** — which is where the CLI-vs-app
cost divergence had been hiding.

```mermaid
flowchart TD
    CT["CostTracker (120 lines)<br/>ObservableObject, cache, demo mode"] --> CSB[CostSummaryBuilder]
    CSB --> CW[CostWindow<br/>window start]
    CSB --> CCS[ClaudeCostScanner]
    CSB --> XCS[CodexCostScanner]
    CCS --> CSV[CostScanValues<br/>coercion + display names]
    CCS --> CFS[CostScanFileSystem<br/>local-volume guard]
    XCS --> CSV
    XCS --> CFS
    CCS --> TCM[TokenCostMath<br/>the one pricing formula]
    XCS --> TCM
    CCS --> TUA[TokenUsageAggregator<br/>daily + breakdown rows]
    XCS --> TUA
    TUA --> TCM
```
...
- **Data/services:** `CostTracker` and eight new files under `MeterBar/Services/`
- **Tests:** new `CostScanCollaboratorTests`; `CostTrackerTests` retargeted at the new namespaces
- **Call sites:** `CostChartPresentation`, `ClaudeFableSessionTracker`, three test files
- **External:** none — no behavior change, no new dependency

### What was done

- [x] Extracted `CostWindow`, `CostScanValues`, `CostScanFileSystem`, `TokenCostMath`,
      `TokenUsageAggregator`, `ClaudeCostScanner`, `CodexCostScanner`, `CostSummaryBuilder`
- [x] Reduced `CostTracker` to 120 lines: publishing, caching, demo mode, cache I/O
- [x] Renamed every call site off the old `CostTracker.` static prefixes
- [x] Widened seven members from `private` to internal, each with a documented reason
- [x] Wrote `CostScanCollaboratorTests` — 26 tests, first direct coverage of the money math
- [x] Fixed three doc comments that still named pre-split symbols

### Key decisions

| Decision | Rationale |
|---|---|
| Plain `enum X` + `nonisolated` on every static member | The app target sets `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor`; per-member `nonisolated` matches the in-repo precedent and keeps the scan off the main actor |
| One namespace per concern, not one `CostScannerHelpers` | Each namespace has a single reason to change; a grab-bag would have re-created the original problem at a smaller scale |
| `TokenCostMath.calculateCost` delegates to `calculateClaudeCost` | The two were near-duplicates that had **drifted** — only the Claude variant clamped negative inputs. One formula now, negative clamping everywhere |
| Widen to internal with a `/// Internal (not private) so …` line | `@testable import` grants `internal`, not `private`. Documenting the reason follows the existing precedent in `CostScanWindows.swift` |
| Byte-faithful moves, except where a prefix crosses 120 chars | A refactor PR that also edits bodies is unreviewable; the only edits are namespace prefixes and line wrapping |
| Kept the single-traversal scan verbatim | Audit O2/O3 (PR #262) halved the scan I/O; `CostSummaryBuilder` preserves the one-`ScanWindows`-fills-both-windows shape exactly |

### Files changed

| File | Change |
|---|---|
| `MeterBar/Services/CostTracker.swift` | 1,018 → 120 lines |
| `MeterBar/Services/CostWindow.swift` | new, 25 |
| `MeterBar/Services/CostScanValues.swift` | new, 35 |
| `MeterBar/Services/CostScanFileSystem.swift` | new, 20 |
| `MeterBar/Services/TokenCostMath.swift` | new, 46 |
| `MeterBar/Services/TokenUsageAggregator.swift` | new, 75 |
| `MeterBar/Services/ClaudeCostScanner.swift` | new, 328 |
| `MeterBar/Services/CodexCostScanner.swift` | new, 388 |
| `MeterBar/Services/CostSummaryBuilder.swift` | new, 60 |
| `MeterBarTests/CostScanCollaboratorTests.swift` | new, 375 |
| `MeterBar/Services/CostScanWindows.swift` | doc comments retargeted |
| `MeterBar/Models/CostChartPresentation.swift`, `MeterBar/Services/ClaudeFableSessionTracker.swift` | call-site renames |
| `MeterBarTests/{CostTracker,CostChartPresentation,ModelPricing}Tests.swift` | call-site renames |

### Verification

- SwiftLint `--strict --quiet` → exit 0
- Declaration diff vs. the pre-split file: all 30 moved declarations accounted for, none invented
- Symbol-uniqueness sweep across all targets: every new type declared exactly once
- Line-length sweep: no new line over 120 (two pre-existing overlong lines in `CostTrackerTests` left untouched)
- Zero remaining references to the pre-split static names; the only `CostTracker.` uses left are `.shared`
- Tests/builds intentionally **not** run locally — PR CI is the gate

### Mistakes and fixes

- A rebase onto the diverged `master` conflicted; instead of force-resolving, the branch was reset to `origin/master` and the split re-derived from master's real file. The stale remote head from the earlier attempt was sidestepped by renaming the local branch and pushing a fresh remote ref — no force-push needed.
- The salvaged test draft called `TokenUsageAggregator.mergeNamedTotals`/`mergeDailyTotals`, which do not exist — master folded them into `ClaudeSessionTotals.merge`. Caught by reading the real file instead of trusting the draft; both tests dropped.
- Landing this after the wave produced a **clean-but-wrong** auto-merge of the day
  file. Git found no textual conflict because this entry and Session 15 were
  inserted at different offsets, but the merged result interleaved them: the
  mermaid fence here was left unclosed, Session 15's `### Affected components`
  swallowed this entry's bullets, and two `## Session 1` headings were stranded as
  bare lines. Fence *parity* still looked plausible; only strict OPEN/CLOSE
  alternation caught it. Resolution was to discard the textual merge of this one
  path, take master's file whole, and re-append this entry as a unit.
- Review caught two latent crashes/leaks the byte-faithful move had carried over
  verbatim from the pre-split file. `CostScanValues.int` fed an arbitrary JSON
  `Double` straight into `Int(_:)`, which traps on NaN, infinities, and anything
  outside `Int`'s range — one malformed line in a third-party CLI's JSONL would
  have taken down the whole refresh. It now saturates. `CodexCostScanner`'s
  SQLite open armed its `defer { sqlite3_close }` *after* the status guard, so a
  failed open leaked the handle SQLite still returns; the close is now armed
  first. A sweep of the other two `sqlite3_open_v2` sites (`CursorLocalService`)
  found both already closing on the failure path — that is the house pattern,
  and only the scanner had drifted from it. Extraction is what made the first
  bug directly assertable; the regression test asserts the saturating bounds.

### Next steps

- Audit finding C1 is complete with this merge (C1a #283, C1b #284, C1c #285, C1d here).
- The money math now has direct coverage for the first time; the CLI-vs-app cost
  divergence should be re-measured against `TokenCostMath` rather than by scanning.
...
- [ ] Require green CI (tests + Xcode build) on the pushed head — `MeterBarApp.swift` compiles only there, and CI is also what confirms the `codex` property/method overload compiles.
